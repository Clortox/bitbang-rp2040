...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    std::cout
        << "An input file with .bin extension must be provided. Exiting ..."
        << std::endl;
    return 1;
  }
  ...
  if (binFilePath.extension() != ".bin") {
    std::cout << "The input file must have .bin extension. Exiting ..."
              << std::endl;
    return 1;
  }
  ...
  if (!std::filesystem::exists(binFilePath)) {
    std::cout << "Could not locate file: " << binFilePath << ". Exiting ..."
              << std::endl;
    return 1;
  }
  ...
  if (binFileSize > 252) {
    std::cout << "The input must be 252 Bytes in size at max. Exiting ..."
              << std::endl;
    binFile.close();
    return 1;
  }
  ...
  *reinterpret_cast<std::uint32_t *>(crc) =
      CRC::Calculate(binFileData, 252, CRC::CRC_32_MPEG2());

  // Output the contents of the crc array to a .c file
  std::ofstream cppFile(
      binFilePath.replace_filename("crc").replace_extension("c"));
